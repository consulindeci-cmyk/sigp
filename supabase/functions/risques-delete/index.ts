...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body: DeleteRisqueBody = await req.json();
    ...
    try {
      ...
      console.error('[risques-delete] historique', historiqueError);
    }

    return json({ message: 'Risque supprimé' });
  } catch (err) {
    const status = (err as { status?: number }).status ?? 500;
    console.error('[risques-delete]', err);
