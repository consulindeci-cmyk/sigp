...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body: DeleteContactBody = await req.json();
    ...
    try {
      ...
      console.error('[governance-contacts-delete] historique', historiqueError);
    }

    return json({ message: 'Contact supprimé' });
  } catch (err) {
    const status = (err as { status?: number }).status ?? 500;
    console.error('[governance-contacts-delete]', err);
